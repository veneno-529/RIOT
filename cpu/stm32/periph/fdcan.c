...
#if defined (CPU_FAM_STM32H7)
/* Caclulate start address for each block in Message RAM */
static void calculate_message_ram_block_addresses(FDCAN_GlobalTypeDef *can)
{
    ...
    can->SIDFC = (can->SIDFC & ~FDCAN_SIDFC_FLSSA)
                | (start_address << FDCAN_SIDFC_FLSSA_Pos);
    /* Don't set LSS value in register yet */
    // can->SIDFC = (can->SIDFC & ~FDCAN_SIDFC_LSS)
    //             | (CONFIG_FDCAN_STD_FILTERS_NUM << FDCAN_SIDFC_LSS_Pos);
    DEBUG("SIDFC=%08lx LSS=%lu FLSSA=0x%lx\n",
        can->SIDFC,
        (can->SIDFC & FDCAN_SIDFC_LSS) >> FDCAN_SIDFC_LSS_Pos,
        (can->SIDFC & FDCAN_SIDFC_FLSSA) >> FDCAN_SIDFC_FLSSA_Pos);

    /* Filter List Extended Start Address */
    start_address += CONFIG_FDCAN_STD_FILTERS_NUM;
    can->XIDFC = (can->XIDFC & ~FDCAN_XIDFC_FLESA)
                | (start_address << FDCAN_XIDFC_FLESA_Pos);
    /* Don't set LSS value in register yet */
    // can->XIDFC = (can->XIDFC & ~FDCAN_XIDFC_LSE)
    //             | (CONFIG_FDCAN_EXT_FILTERS_NUM << FDCAN_XIDFC_LSE_Pos);

    /* Rx FIFO0 Start Address */
    start_address += CONFIG_FDCAN_EXT_FILTERS_NUM * 2U;
    can->RXF0C = (can->RXF0C & ~FDCAN_RXF0C_F0SA)
                | (start_address << FDCAN_RXF0C_F0SA_Pos);
    ...
    can->RXF1C = (can->RXF1C & ~FDCAN_RXF1C_F1SA)
                | (start_address << FDCAN_RXF1C_F1SA_Pos);
    ...
    can->RXBC = (can->RXBC & ~FDCAN_RXBC_RBSA)
                | (start_address << FDCAN_RXBC_RBSA_Pos);
    ...
    can->TXEFC = (can->TXEFC & ~FDCAN_TXEFC_EFSA)
                | (start_address << FDCAN_TXEFC_EFSA_Pos);
    ...
    can->TXBC = (can->TXBC & ~FDCAN_TXBC_TBSA)
                | (start_address << FDCAN_TXBC_TBSA_Pos);
    ...
    DEBUG("TXBC=%08lx NDTB=%lu TFQS=%lu TBSA=0x%lx\n",
      can->TXBC,
      (can->TXBC & FDCAN_TXBC_NDTB) >> FDCAN_TXBC_NDTB_Pos,
      (can->TXBC & FDCAN_TXBC_TFQS) >> FDCAN_TXBC_TFQS_Pos,
      (can->TXBC & FDCAN_TXBC_TBSA) >> FDCAN_TXBC_TBSA_Pos);
...
uint32_t* rx_fifo_element_r0;
    ...
    else
    {
        rx_fifo_element_r0 =
            get_message_ram_rx_fifo_address(can, message_ram_rx_fifo) +
            (get_index * FDCAN_RXFIFO_ELEMENT_SIZE_WORDS);
    }
#else
    rx_fifo_element_r0 =
        get_message_ram_rx_fifo_address(can, message_ram_rx_fifo) +
